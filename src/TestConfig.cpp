...
Rideable* GlobalTestConfig::allocRideable(int rideableType_){
	Rideable* r = rideableFactories[rideableType_]->build(this);
	allocatedRideables.push_back(r);
	return r;
}
Rideable* GlobalTestConfig::allocRideable(){
	return allocRideable(rideableType);
}

void GlobalTestConfig::setUpEpochSys(void* esys_){
	_preallocated_esys = true;
	_esys = esys_;
}

void GlobalTestConfig::setUpTDSLTxns(void* tdsl_txns_){
	_preallocated_tdsl_txns = true;
	_tdsl_txns = tdsl_txns_;
}

int GlobalTestConfig::findRideable(std::string rideableName_){
	int ret = -1;
	auto idx = std::find(rideableNames.begin(), rideableNames.end(), rideableName_);
	if (idx == rideableNames.end()){
		fprintf(stderr, "Invalid rideable name.\n");
		printargdef();
	} else {
		ret = idx - rideableNames.begin();
	}
	return ret;
}
	...
	while ((c = getopt (argc, argv, "s:d:w:o:i:t:m:M:a:r:R:vhz")) != -1){
		switch (c) {
			...
			case 'R':
				{
					std::string n(optarg);
					this->rideableType = findRideable(n);
				}
				break;
			case 'a':
...
void GlobalTestConfig::buildAffinity(std::vector<hwloc_obj_t>& aff){
	...
	assert(aff.size() >= (size_t)task_num);
}
...
GlobalTestConfig::~GlobalTestConfig(){
	...
	if (_preallocated_esys)
		delete reinterpret_cast<pds::EpochSys*>(_esys);
}
...
std::string GlobalTestConfig::getRideableName(int rideableType_){
	return rideableNames[rideableType_];
}
std::string GlobalTestConfig::getRideableName(){
	return getRideableName(this->rideableType);
}
