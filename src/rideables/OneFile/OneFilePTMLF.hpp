...
static const uint64_t PREGION_SIZE = 10*1024*1024*1024ULL;   // 1 GB by default
...
public:
    void init(void* addressOfMemoryPool, size_t sizeOfMemoryPool, bool clearPool=true) {
        ...
        if (clearPool) {
            // Wentao: Comment out memset to avoid long start time
...
struct WriteSet {
    ...
    WriteSet() {
        numStores = 0;
        for (uint64_t i = 0; i < HASH_BUCKETS; i++) buckets[i] = &log[TX_MAX_STORES-1];
    }
...
class OneFileLF {
    template <class T>
    friend struct tmtype;
private:
    uint8_t                             *regionAddr = PREGION_ADDR;
    uint8_t                             *regionEnd = PREGION_END;
...
public:
    ...
    OneFileLF() {
        ...
        mapPersistentRegion(PFILE_NAME, PREGION_SIZE);
    }
    ...
    void mapPersistentRegion(const char* filename, const uint64_t regionSize) {
        ...
        if (got_addr == MAP_FAILED) {
            ...
            assert(false);
        } else if (got_addr != regionAddr) {
            // printf("got_addr = %p  instead of %p\n", got_addr, regionAddr);
            regionAddr = (uint8_t*)got_addr;
            regionEnd = regionAddr+PREGION_SIZE;
        }
...
template<typename T> struct tmtype : tmtypebase<T> {
    ...
    inline T pload() const {
        ...
        if ((uint8_t*)this < gOFLF.regionAddr || (uint8_t*)this > gOFLF.regionEnd) return lval;
