...
#define SET_MARK(_p)    ((transskip_node_t *)(((uintptr_t)(_p)) | 1))
 ...
 */
static transskip_node_t *alloc_node(ptst_t *ptst)
{
    int l;
    transskip_node_t *n;
    l = get_level(ptst);
    n = (transskip_node_t*)fr_gc_alloc(ptst, gc_id[l - 1]);
    n->level = l;
...
static void free_node(ptst_t *ptst, transskip_node_t* n)
{
 ...
 */
static transskip_node_t* strong_search_predecessors(trans_skip *l, setkey_t k, transskip_node_t* *pa, transskip_node_t* *na)
{
    transskip_node_t* x, *x_next, *old_x_next, *y, *y_next;
    setkey_t  y_k;
    int        i;
    ...
    for ( i = NUM_LEVELS - 1; i >= 0; i-- )
    {
        ...
        for ( y = x_next; ; y = y_next )
        {
            ...
            for ( ; ; )
            {
                ...
                y = (transskip_node_t*)get_unmarked_ref(y_next);
            }
...
static transskip_node_t* weak_search_predecessors(trans_skip *l, setkey_t k, transskip_node_t* *pa, transskip_node_t* *na)
{
    transskip_node_t* x, *x_next;
    setkey_t  x_next_k;
    int        i;
    ...
    for ( i = NUM_LEVELS - 1; i >= 0; i-- )
    {
        ...
        {
            READ_FIELD(x_next, x->next[i]);
            x_next = (transskip_node_t*)get_unmarked_ref(x_next);
 ...
 * forward pointers.
 */
static void mark_deleted(transskip_node_t* x, int level)
{
    transskip_node_t* x_next;
...
static int check_for_full_delete(transskip_node_t* x)
{
    int level = x->level;
...
static void do_full_delete(ptst_t *ptst, trans_skip *l, transskip_node_t* x, int level)
{
    int k = x->k;
#ifdef WEAK_MEM_ORDER
    transskip_node_t* preds[NUM_LEVELS];
    int i = level;
 retry:
    ...
    while ( i > 0 )
    {
        transskip_node_t *n = get_unmarked_ref(preds[i]->next[i]);
        while ( n->k < k )
        {
...
trans_skip *transskip_alloc(Allocator<Desc>* _descAllocator, Allocator<NodeDesc>* _nodeDescAllocator)
{
    ...
    n = (transskip_node_t*)malloc(sizeof(*n) + (NUM_LEVELS-1)*sizeof(transskip_node_t *));
    memset(n, 0, sizeof(*n) + (NUM_LEVELS-1)*sizeof(transskip_node_t *));
    n->k = SENTINEL_KEYMAX;
     ...
     */
    memset(n->next, 0xfe, NUM_LEVELS*sizeof(transskip_node_t *));

    l = (trans_skip*)malloc(sizeof(*l) + (NUM_LEVELS-1)*sizeof(transskip_node_t *));
...
bool transskip_insert(trans_skip *l, setkey_t k, setval_t v, Desc* desc, uint8_t opid, transskip_node_t*& n)
{
    n = NULL;
    bool ret = false;
    ...
    ptst_t    *ptst;
    transskip_node_t* preds[NUM_LEVELS], *succs[NUM_LEVELS];
    transskip_node_t* pred, *succ, *new_node = NULL, *new_next, *old_next;
    int        i, level;
...
bool transskip_delete(trans_skip *l, setkey_t k, Desc* desc, uint8_t opid, transskip_node_t*& n)
{
    n = NULL;
    bool ret = false;
...
setval_t transskip_delete_org(trans_skip *l, setkey_t k)
{
    setval_t  v = NULL;
    ptst_t    *ptst;
    transskip_node_t* preds[NUM_LEVELS], *x;
    int        level, i;
...
void init_transskip_subsystem(void)
{
    ...
    for ( i = 0; i < NUM_LEVELS; i++ )
    {
        gc_id[i] = fr_gc_add_allocator(sizeof(transskip_node_t) + i*sizeof(transskip_node_t *));
    }
}
...
static inline bool help_ops(trans_skip* l, Desc* desc, uint8_t opid)
{
    ...
    //std::vector<transskip_node_t*> deletedNodes;
    //std::vector<transskip_node_t*> insertedNodes;
    ...
    if(ret == true)
    {
        if(__sync_bool_compare_and_swap(&desc->status, LIVE, COMMITTED))
        {
            ...
            //{
                //transskip_node_t* x = deletedNodes[i];
                //if(x == NULL) { continue; }
    ...
    else
    {
        if(__sync_bool_compare_and_swap(&desc->status, LIVE, ABORTED))
        {
            ...
            //{
                //transskip_node_t* x = insertedNodes[i];
                //if(x == NULL) { continue; }
